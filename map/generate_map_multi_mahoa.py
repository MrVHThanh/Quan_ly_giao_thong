"""
generate_map_multi.py  (v2 — bảo vệ dữ liệu tọa độ)
------------------------------------------------------
...
Output gồm 2 file:
  • ban_do.html        — giao diện bản đồ (không chứa tọa độ thô)
  • routes_data.js     — dữ liệu đã mã hóa (cần có cả 2 file mới chạy được)

Cách dùng:
    python generate_map_multi.py
    python generate_map_multi.py --input QL4E_merged.geojson TL158_merged.geojson
    python generate_map_multi.py --output ban_do.html --data routes_data.js
"""
...
DEFAULT_HTML   = "Giao_thong_map_XXX.html"
DEFAULT_DATA   = "routes_data.js"
...
# ══════════════════════════════════════════════════════════════════
#  SINH FILE DỮ LIỆU RIÊNG (routes_data.js)
# ══════════════════════════════════════════════════════════════════

def generate_data_js(route_metas, key: bytes, data_filename: str) -> str:
    """
    File JS chứa:
      - Key (dạng Uint8Array)
      - Dữ liệu đã mã hóa của từng tuyến
    File này cần được <script src="..."> trong HTML.
    Không có file này → bản đồ không hiện tuyến đường.
    """
    key_array = list(key)
    encoded_routes = []
    ...
    return f"""/* routes_data.js — Dữ liệu tuyến đường (đã mã hóa)
 * Được tạo tự động bởi generate_map_multi.py
 * File này cần đi kèm với file HTML để bản đồ hoạt động.
 * Dữ liệu tọa độ được mã hóa 3 lớp: Delta + XOR + Base64.
 */
...
def generate_html(route_metas, data_filename: str, key: bytes) -> str:
...
<head>
  ...
  <!-- File dữ liệu tuyến đường (đã mã hóa) -->
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    /* ── Thông báo lỗi thiếu file data ── */
    #data-error {{
      display: none; position: fixed; inset: 0; background: var(--bg);
      align-items: center; justify-content: center; z-index: 9999; flex-direction: column; gap: 16px;
    }}
    #data-error.visible {{ display: flex; }}
    .err-box {{
      background: var(--panel); border: 1px solid #f87171; border-radius: 12px;
      padding: 32px 40px; max-width: 420px; text-align: center;
    }}
    .err-box h2 {{ color: #f87171; font-size: 18px; margin-bottom: 12px; }}
    .err-box p  {{ color: var(--muted); font-size: 13px; line-height: 1.6; }}
    .err-box code {{ background: rgba(255,255,255,.07); padding: 2px 6px; border-radius: 4px; color: var(--accent); font-size: 12px; }}
...
<!-- Thông báo nếu thiếu file data -->
<div id="data-error">
  <div class="err-box">
    <h2>⚠️ Không tìm thấy dữ liệu tuyến đường</h2>
    <p>File <code>{data_filename}</code> bị thiếu hoặc chưa được load.<br><br>
       Hãy đảm bảo file <code>{data_filename}</code> nằm <strong>cùng thư mục</strong> với file HTML này.</p>
  </div>
</div>
...
<script>
// ── Kiểm tra file data có được load không ───────────────────────
if (typeof ROUTES_ENC === 'undefined' || typeof _ROUTE_KEY === 'undefined') {{
  document.getElementById('data-error').classList.add('visible');
  document.getElementById('loader').classList.add('hidden');
  throw new Error('Thiếu file dữ liệu: {data_filename}');
}}
...
def main():
    parser = argparse.ArgumentParser(
        description="Tạo bản đồ HTML từ nhiều GeoJSON — dữ liệu tọa độ được mã hóa"
    )
    ...
    parser.add_argument("--data",   "-d", default=DEFAULT_DATA,
        help=f"File JS dữ liệu đầu ra (mặc định: {DEFAULT_DATA})")
    ...
    # Sinh file dữ liệu JS
    print(f"\n🔐 Đang mã hóa tọa độ (Delta → XOR → Base64)...")
    data_js = generate_data_js(routes, key, args.data)
    with open(args.data, "w", encoding="utf-8") as f:
        f.write(data_js)
    print(f"   ✅ File dữ liệu : {args.data}  ({len(data_js)/1024:.1f} KB)")

    # Sinh file HTML
    html = generate_html(routes, args.data, key)
    with open(args.output, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"   ✅ File HTML     : {args.output}  ({len(html)/1024:.1f} KB)")

    print(f"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ Hoàn thành! Cần 2 file để bản đồ hoạt động:

   📄 {args.output}    ← mở file này bằng trình duyệt
   📦 {args.data}   ← file dữ liệu (KHÔNG chia sẻ nếu muốn bảo vệ)

⚠️  Nếu chỉ gửi file HTML mà không có {args.data}
    → bản đồ sẽ hiện thông báo lỗi, không hiện tuyến đường.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━""")
